---
kind: tutorial
area: build
title: "Write your first gamemode"
description: "From an empty directory to a gamemode that places players, in seven files. Every step here was run against the engine, and the log output is the real one."
sidebar:
  label: "First gamemode"
  order: 4
---

import { FileTree, Steps, Aside } from '@astrojs/starlight/components';

A gamemode is an ordinary mod that the server designates as the session's rule layer. The
smallest useful one takes over spawning: it tells the host to stop placing players, then
places them itself.

## The files

Under the game's content root — `assets/workshop/` beside the install — you are creating a
author segment, an addon, and a mod directory. Your mod will be `tutorial:hello:hellomode` —
author, addon, mod, one part per level: **identity comes from the path**, never from a
manifest, so nothing below repeats the name.

<FileTree>
- workshop
  - tutorial your author segment
    - hello an addon
      - addon.toml marks this an addon root
      - **hellomode** the mod — these three names ARE its identity
        - mod.toml
        - Cargo.toml
        - .cargo/config.toml
        - wit
          - world.wit
          - deps/ironlark-host/host.wit a copy of the host's contract
        - server
          - Cargo.toml
          - src/lib.rs
</FileTree>

<Steps>

1. **Mark the addon root** — `addon.toml`

   ```toml
   [addon]
   version = "0.1.0"
   ```

2. **Declare the mod** — `mod.toml`

   The `roles` line is what makes this a gamemode candidate rather than an ordinary mod.

   ```toml
   [mod]
   version = "0.1.0"

   [declares]
   roles = ["gamemode"]
   ```

3. **Set up the workspace** — `Cargo.toml`

   A workspace rather than a plain crate, because a mod may later grow a `client/` half
   beside the server one.

   ```toml
   [workspace]
   resolver = "2"
   members = ["server"]

   [workspace.package]
   version = "0.1.0"
   edition = "2024"

   [workspace.dependencies]
   wit-bindgen = "0.58"

   [profile.release]
   opt-level = "z"
   lto = true
   strip = "debuginfo"
   ```

4. **Pin the target** — `.cargo/config.toml`

   ```toml
   [build]
   target = "wasm32-wasip2"
   ```

   :::caution[Not optional convenience]
   Without this a plain `cargo build` targets your native platform, and the async export
   symbols the bindings generate break the native linker rather than failing with anything
   helpful.
   :::

5. **Name your WIT package** — `wit/world.wit`

   ```wit
   package tutorial:hellomode@0.1.0;
   ```

6. **Vendor the host contract** — `wit/deps/ironlark-host/host.wit`

   A **copy** of the contract your mod compiles against: [download it here](/host.wit).

   :::note[Keep it in step with the game]
   It must match the version of the game you run against. A stale copy fails to
   instantiate without a useful message.
   :::

7. **Declare the server crate** — `server/Cargo.toml`

   ```toml
   [package]
   name = "hellomode-server"
   version.workspace = true
   edition.workspace = true

   [lib]
   crate-type = ["cdylib"]

   [dependencies]
   wit-bindgen = { workspace = true }
   ```

</Steps>

## The code

`server/src/lib.rs`. The interesting part is eleven lines; the rest is the contract.

```rust
wit_bindgen::generate!({
    path: "../wit",
    world: "ironlark:host/server-mod",
});

use std::sync::atomic::{AtomicUsize, Ordering};

use exports::ironlark::host::server_api::Guest;
use ironlark::host::entity::{SpawnTransform, Vec3, control, spawn};
use ironlark::host::gamemode::set_default_spawn;
use ironlark::host::log::{Level, log};
use ironlark::host::map_api::list_spawns;

struct Component;

/// Spread joiners over the map's spawn points instead of stacking them.
static NEXT: AtomicUsize = AtomicUsize::new(0);

impl Guest for Component {
    async fn init() {
        // We place players, so the host must stop doing it. In init, because the
        // host holds joins until every server mod has loaded — do this later and
        // the first player is already spawned.
        set_default_spawn(false);
        log(Level::Info, "hellomode: init, we own spawn now");
    }

    async fn on_player_join(player_id: String) {
        let spawns = list_spawns().await; // never empty
        let point = &spawns[NEXT.fetch_add(1, Ordering::Relaxed) % spawns.len()];
        let at = SpawnTransform {
            position: Vec3 {
                x: point.position.x,
                y: point.position.y,
                z: point.position.z,
            },
            yaw: point.yaw,
        };

        // Two steps, always: an entity exists, then someone drives it.
        let body = match spawn("character".into(), at).await {
            Ok(body) => body,
            Err(e) => {
                log(Level::Error, &format!("hellomode: spawn failed: {e}"));
                return;
            }
        };
        if let Err(e) = control(player_id.clone(), &body).await {
            log(Level::Error, &format!("hellomode: control failed: {e}"));
            return;
        }
        log(
            Level::Info,
            &format!("hellomode: spawned and controlled a body for {player_id}"),
        );
    }

    // Everything below is required by the contract and does nothing here. A
    // component missing one of them does not instantiate at all.

    async fn on_player_leave(_player_id: String) {}

    async fn update(_dt: f32) {}

    async fn on_interact(
        _player_id: String,
        _target: String,
        _hit_point: exports::ironlark::host::server_api::Vec3,
        _distance: f32,
    ) {
    }

    async fn on_signal(_channel: String, _source: String, _payload: Vec<u8>) {}

    async fn on_contact(
        _target: String,
        _other: exports::ironlark::host::server_api::ContactParty,
        _point: exports::ironlark::host::server_api::Vec3,
        _edge: exports::ironlark::host::server_api::ContactEdge,
    ) {
    }

    async fn handle_rpc(
        _player_id: String,
        method: String,
        _args: Vec<u8>,
    ) -> Result<Vec<u8>, String> {
        Err(format!("unknown method: {method}"))
    }
}

export!(Component);
```

Note the shape of the two calls that matter: **`spawn` then `control`**, never one fused
call. The entity exists first; a controller is bound to it second. That separation is what
makes bodiless players, spectators and possession possible at all.

## Build it

```bash
cargo build --release            # from the mod directory
```

You should get `target/wasm32-wasip2/release/hellomode_server.wasm`.

:::caution[Do not put a hyphen in the mod directory name]
The host looks for `<mod-directory>_server.wasm`. Cargo names its artifact after the crate,
turning hyphens into underscores — so a directory called `hello-gamemode` produces
`hello_gamemode_server.wasm` while the host looks for `hello-gamemode_server.wasm`, and never
finds it.

The host says so plainly, as an `ERROR` naming the path it wanted and the command that
builds it:

```
mod tutorial:hello:hello-gamemode has a server realm that is not built: no component at
  .../hello-gamemode/target/wasm32-wasip2/release/hello-gamemode_server.wasm. Build it
  with: cargo build --release --target wasm32-wasip2 --manifest-path
  .../hello-gamemode/Cargo.toml
```

Running that command will not help, because cargo will write the underscored name again.
The fix is the directory: keep mod directory names free of hyphens.

The consequence is worth knowing anyway. The mod is still discovered and can still be
designated the gamemode, so the gamemode you named is loaded as nothing while the one you
did not name was dropped for not being chosen — and nobody spawns at all.
:::

## Run it

Your gamemode and the bundled `ironlark:core:freeroam` are now both candidates, so the session refuses
to start until you name one — that refusal is the resolver working, not a bug:

Name yours in `config/server.toml` beside the game:

```toml
[session]
gamemode = "tutorial:hello:hellomode"
```

Then host a session from the launcher as usual. The addon has to be in the content root the
game reads — `assets/workshop/` beside the install — which is where you built it in the
first step.

## What success looks like

In `logs/host.log`:

```
workshop: 10 mod(s) — … tutorial:hello:hellomode 0.1.0
gamemode: tutorial:hello:hellomode
hellomode: init, we own spawn now
gamemode: 'tutorial:hello:hellomode' set default-spawn = false
server-mod loaded: tutorial:hello:hellomode
spawn: spawned 'character' body 358v0 at Vec3(0.0, 5.0, 12.0) for 'tutorial:hello:hellomode' (uncontrolled)
control: 'tutorial:hello:hellomode' bound local player to body 358v0
hellomode: spawned and controlled a body for af74f667-...
```

The body is announced `uncontrolled` and becomes controlled a line later — that is the two
steps, in the log.

You will also see `has no client half`, which is correct: this mod has no client half. Add
one when you need something on a player's screen.

## Where to go next

- Rounds, phases and per-player HUD text: [Gamemodes](/build/gamemodes/) and
  [Broadcast and RPC](/build/broadcast-and-rpc/)
- Zones and trigger volumes, for anything area-based: [Contact events](/build/contact-events/)
- Every function you can call: [Interface reference](/reference/)
- Before designing something ambitious: [What you cannot build yet](/boundary/)
