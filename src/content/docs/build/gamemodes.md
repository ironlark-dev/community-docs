...
description: "A gamemode is an ordinary mod the server designates as the session's baseline rule layer. The engine has no gamemode concept beyond that one slot."
sidebar:
  label: "Gamemodes"
  order: 40
---
...
yet](/boundary/): a player with no body has no camera, so waiting is currently a black
...
renders it — see [broadcast and RPC](/build/broadcast-and-rpc/). A server-only gamemode cannot
